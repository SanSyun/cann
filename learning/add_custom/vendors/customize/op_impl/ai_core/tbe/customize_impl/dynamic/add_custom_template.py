...
def add_custom_template(x_in__, y_in__, z_out_, kernel_name="add_custom_template", impl_mode = ""):
    ...
    ascendc_src_dir_ex = "add_custom_template"
    ascendc_src_dir = "add_custom_template"
    ...
    if src_file != "":
        ...
        ascendc_src_file = "add_custom_template.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AddCustomTemplate, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
...
def op_select_format(x_in__, y_in__, z_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_)
    result = check_op_cap("op_select_format", "AddCustomTemplate", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, z_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_)
    result = check_op_cap("get_op_specific_info", "AddCustomTemplate", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
